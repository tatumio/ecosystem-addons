...
describe('TronWalletProvider', () => {
  ...
  const mnemonic =
    'chat pledge host breeze follow joke wonder vendor glance attract skull easy obey devote pave peace estate alone increase plastic major glow deliver addict'
  const xpub =
    'xpub6FLDUJoYnwKtLyq2DiXLUJ6a6GTacCLKgz3F3t1AH5ncbDrUnyKCkPaSwTh2Qq5Xzwrwk2Bnw8aDktmXXhgoGPWQE5eEiN8vyQSaXuq7s5s'
  ...
  describe('generateXpub', () => {
    it('should generate xpub from mnemonic', async () => {
      const result = await tatumSdk.walletProvider.use(TronWalletProvider).generateXpub(mnemonic)
